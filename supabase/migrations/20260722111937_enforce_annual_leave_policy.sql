-- Annual leave policy for LOEI SMART GROUP.
-- Personal leave and annual vacation are calendar-year entitlements with no carry-over.
-- Vacation is available after the employee completes one full year of service.

create index if not exists leave_requests_employee_type_status_start_idx
  on public.leave_requests (employee_id, leave_type, status, start_date)
  include (days);
...
declare
  ...
  other_approved_days integer := 0;
  previous_target_total integer := 0;
...
begin
  ...
  select coalesce(sum(request.days), 0)::integer
...
comment on function private.enforce_annual_leave_policy() is
  'Enforces Owner final approval, six-day annual personal/vacation quotas, one-year vacation eligibility, and calendar-year expiry.';
